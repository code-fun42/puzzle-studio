        ...
        <div class="container flex justify-between items-center mx-auto sm:py-2 py-1 header__container">
            <a href="#" class="header__logo-link">
                <picture class="header__logo-picture">
                    <source srcset="../assets/img/header-logo-mobile.svg" media="(max-width: 640px)"
                        type="image/svg+xml">
            ...
            <div class="z-10 lg:hidden flex w-[40px] h-[36px] header__burger">
                <button v-on:click="burger" class="relative w-[40px] h-[36px] header__burger-btn">
                    <span
                        class="absolute top-[0px] flex w-[40px] h-1 header__burger-line  header__burger-line-1"></span>
                    <span
                        class="absolute top-[16px] flex right-[0px] w-[40px] h-1 header__burger-line header__burger-line-2"></span>
                    <span
                        class="absolute top-[32px] flex w-[40px] h-1 header__burger-line header__burger-line-3"></span>
                </button>
            </div>
            <nav
                ...
                <ul class="flex flex-col items-start header__burger-list">
                    <li v-for="item in menuArr" class="mb-[20px] header__burger-item">
                        <a v-bind:href="item.href" class="header__burger-link link-scroll">{{ item.text }}</a>
    ...
    methods: {
        burger() {
            if (!this.burgerStatus) {
                ...
                // const burgerMenuWidth = +$burgerMenu.getStyle().width.split('p')[0]
